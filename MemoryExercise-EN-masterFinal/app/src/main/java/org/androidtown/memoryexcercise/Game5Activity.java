...
public class Game5Activity extends AppCompatActivity {
    ...
    String[] number = {"①", "②", "③", "④"};
    String ans = "";
    String[] answer = new String[3]; // change
    int num = 1;
    int count = 0, currentPoint;
    int timerVal;
    ...
    protected void onStart() {
        ...
        // 3개의 알파벳을 random 으로 고른다. 0 ~ 25

        for (int i = 0; i < 3; i++) {
            randNum[i] = random.nextInt(26);
            answer[i] = alphabet[randNum[i]]; // change
            ans += answer[i]; // change
        }
...
//                }
    ...
    void memoryTest() {
        String suggestion = answer[0] + alphabet[random.nextInt(26)] + alphabet[random.nextInt(26)];
        int chance;
    ...
    public void setStar(int current){
        ...
        toEdit = sh.edit();
        star = sh.getInt("game5level1", 0);
        if(current > star)
            toEdit.putInt("game5level1",current);
        toEdit.commit();
